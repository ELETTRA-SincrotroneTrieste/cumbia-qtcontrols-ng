...
CuApplyNumericClickRepeater::CuApplyNumericClickRepeater(QObject *parent) : QObject{parent} {
    d  = new CuANCRPrivate;
    d->pos = QPointF();
    d->repecnt = 0;
}

CuApplyNumericClickRepeater::~CuApplyNumericClickRepeater() {
    foreach(QTimer *t, findChildren<QTimer *>())
        delete t;
    delete d;
}

bool CuApplyNumericClickRepeater::active() const {
    return findChild<QTimer *>("t2") != nullptr;
}

void CuApplyNumericClickRepeater::pressed(const QPointF &pos)
{
    d->repecnt = 0;
    d->pos = pos;
...
void CuApplyNumericClickRepeater::released(const QPointF &)
{
    d->repecnt = 0;
    d->pos = QPointF();
...
void CuApplyNumericClickRepeater::m_start_repeat() {
    if(sender())
        delete sender();
...
void CuApplyNumericClickRepeater::m_on_repeat_timeout()
{
    if(!d->pos.isNull()) {
        emit step(d->pos);
        d->repecnt++;
